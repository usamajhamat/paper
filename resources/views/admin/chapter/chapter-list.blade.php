...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-12">
            <form class="card" id="searchForm" method="post" action="{{route('chapter-search')}}" enctype="multipart/form-data">
                @csrf
                <div class="card-header">
                    <h3 class="card-title">Search</h3>
                    <div class="card-options">
                        <a href="{{route('chapter-add')}}" id="add__new__list" type="button" class="btn btn-sm btn-success "><i class="fa fa-plus"></i> Add Chapter</a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
    ...
    <div class="row">
        <div class="col-md-12 col-lg-12">
            <div class="card" style="display:none;" id="dt">
                ...
                <div class="card-body" id="chapterData">
                </div>
                <!-- table-wrapper -->
            </div>
            <!-- section-wrapper -->
    ...
    <div class="modal fade" id="chapterModal" tabindex="-1" role="dialog" aria-labelledby="chapterModal" aria-hidden="true">
        <div class="modal-dialog  modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="classModal">Edit Chapter</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form  id="editForm" method="post" action="{{route('chapter-update')}}" enctype="multipart/form-data">
                        @csrf
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="editChapterBtn" class="btn btn-primary">Save changes</button>
                </div>
            </div>
        </div>
    </div>
...
@push('script')
    <script>
        $(document).ready(function () {
            ...
            $(document).on('submit', '#searchForm', function (e) {
                ...
                $.ajax({
                    ...
                    success:function(data){
                        $('#dt').show();
                        $('#chapterData').html(data.chapters)
                    },
            ...
            $(document).on('click', '#editChapterBtn', function (e) {
                e.preventDefault();
                if($('#editForm').valid())
                {
                    $.ajax({
...
@push('script')
    <script>
        $(document.body).on('click', '#editChapterBtn', function(){
            $('#editForm').validate(
                {
                    rules: {
                        course: {
